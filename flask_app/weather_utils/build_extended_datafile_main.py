...
def build_extended_datafile_main(csv_name, path):
    # Import CSV file
    file = f'{path}/clean_data/{csv_name}.csv'
    df = pd.read_csv(file)
    ...
    if file_invalid:
        ...
        df_extended.to_csv(f'{path}/clean_data_extended/{csv_name}_Extended.csv', index=False)


def append_to_extended_datafile_main(csv_name, path):
    # Import CSV file
    file = f'{path}/clean_data_extended/{csv_name}_Extended.csv'
    ...
    running_chunk = False
    row_counter = 0
    get_elevation = False  # Assuming elevation data already exists in the extended file
    file_invalid = False

    for index, row in tqdm(df_extended.iterrows(), total=len(df_extended)):
        # Convert timestamp to local time
        dt_local = datetime.strptime(row['ts'], '%Y-%m-%dT%H:%M:%S')
        local_time_str = dt_local.strftime('%Y-%m-%dT%H:%M')
    ...
    if file_invalid:
        ...
        df_extended.to_csv(file, index=False)
